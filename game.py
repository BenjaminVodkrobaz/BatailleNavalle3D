...
class Game:
    ...
    def gameloop(self):
        ...
        print(
            f"ET LE GAGNANT EST LE JOUEUR {self.joueur_actuel_code} ! BIEN JOUE JOUEUR {self.joueur_actuel_code}, TU AS ETE LE PLUS FORT !! \n\n\n\n\n ENCORE B-R-A-V-O, JOUEUR {self.joueur_actuel_code}.")
    ...
    def update_plateau_prime(self, coord_tir, etat_tir):
        ...
        if etat_tir == Etat_Tir.R:
            try:
                if self.prochain_joueur.player_grid[coord_tir["z"] + 1][coord_tir["y"]][coord_tir["x"]] is not None \
                        and flag_vu == False:
                    flag_vu = True
            except IndexError:
                pass
            ...
            try:
                if self.prochain_joueur.player_grid[coord_tir["z"]][coord_tir["y"] + 1][coord_tir["x"]] is not None \
                        and flag_vu == False:
                    flag_vu = True
            except IndexError:
                pass
            try:
                if self.prochain_joueur.player_grid[coord_tir["z"]][coord_tir["y"]][coord_tir["x"] + 1] is not None \
                        and flag_vu == False:
                    flag_vu = True
            except IndexError:
                pass
            ...
            try:
                ...
                pass
            try:
                if self.joueur_actuel.player_prim_grid[coord_tir["z"]][coord_tir["y"]][coord_tir["x"]] is None \
                        or self.joueur_actuel.player_prim_grid[coord_tir["z"]][coord_tir["y"]][
                    coord_tir["x"]] == Etat_Tir.R:
                ...
                if flag_vu:
                    try:
                        if self.joueur_actuel.player_prim_grid[coord_tir["z"] + 1][coord_tir["y"]][
                            coord_tir["x"]] is None \
                                or self.joueur_actuel.player_prim_grid[coord_tir["z"] + 1][coord_tir["y"]][
                                coord_tir["x"]] == Etat_Tir.R:
                            self.joueur_actuel.player_prim_grid[coord_tir["z"] + 1][coord_tir["y"]][
                                coord_tir["x"]] = Etat_Tir.V
                    except IndexError:
                        pass
                    try:
                        if self.joueur_actuel.player_prim_grid[coord_tir["z"]][coord_tir["y"] + 1][
                            coord_tir["x"]] is None \
                                or self.joueur_actuel.player_prim_grid[coord_tir["z"]][coord_tir["y"] + 1][
                                coord_tir["x"]] == Etat_Tir.R:
                            self.joueur_actuel.player_prim_grid[coord_tir["z"]][coord_tir["y"] + 1][
                                coord_tir["x"]] = Etat_Tir.V
                    except IndexError:
                        pass
                    try:
                        if self.joueur_actuel.player_prim_grid[coord_tir["z"]][coord_tir["y"]][
                            coord_tir["x"] + 1] is None \
                                or self.joueur_actuel.player_prim_grid[coord_tir["z"]][coord_tir["y"]][
                                coord_tir["x"] + 1] == Etat_Tir.R:
                            self.joueur_actuel.player_prim_grid[coord_tir["z"]][coord_tir["y"]][
                                coord_tir["x"] + 1] = Etat_Tir.V
                    except IndexError:
                        pass
                    try:
                        if self.joueur_actuel.player_prim_grid[coord_tir["z"] - 1][coord_tir["y"]][
